...
describe('ChromeBrowserAdapter', () => {
  ...
  it('captures screenshot', async () => {
    vi.mocked(chrome.tabs.captureVisibleTab as any).mockResolvedValue('data:image/png;base64,abc');
  ...
  it('sends message', async () => {
    vi.mocked(chrome.runtime.sendMessage as any).mockResolvedValue({ success: true });
  ...
  it('creates tab', async () => {
    ...
    vi.mocked(chrome.tabs.create).mockResolvedValue(mockTab as any);
  ...
  it('removes tab', async () => {
    vi.mocked(chrome.tabs.remove).mockResolvedValue(undefined as any);
  ...
  it('attaches debugger', async () => {
    vi.mocked(chrome.debugger.attach).mockResolvedValue(undefined as any);
  ...
  it('detaches debugger', async () => {
    vi.mocked(chrome.debugger.detach).mockResolvedValue(undefined as any);
  ...
  it('sends debugger command', async () => {
    vi.mocked(chrome.debugger.sendCommand as any).mockResolvedValue({ nodeId: 42 });
...
describe('ChromeStorageProvider', () => {
  ...
  it('gets value from storage', async () => {
    vi.mocked(chrome.storage.local.get as any).mockResolvedValue({ myKey: 'myValue' });
