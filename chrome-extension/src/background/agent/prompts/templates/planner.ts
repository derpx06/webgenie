...
4. Otherwise set done=false, choose the macro_objective for the next phase, and write next_goal as one concrete sentence that keeps exact values from the task (names, emails, URLs, quoted text). Never fill in a value the task, the user's answers and the page do not give (a phone number, an email, an address, which of several differing items): choose ASK_HUMAN for it instead.
