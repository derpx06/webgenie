...
//   E2E_SUITE=core|complex|hitl|security|breadth|endurance|resilience|memory|files|vision|all (default core)   E2E_ONLY=T1,C13   E2E_REPEAT=2   E2E_HEADLESS=1
...
/** Files as the side panel sends them to the background: base64 bytes. */
const encodeFiles = files => files?.map(({ name, type, content }) => ({ name, type, data: Buffer.from(content).toString('base64') }));

/** The message text the side panel builds for attached files (a text file's contents, or a note for a binary one). */
function withAttachments(text, files) {
  if (!files?.length) return text;
  const blocks = files.map(({ name, type, content }) => {
    const bytes = Buffer.from(content);
    let shown = '';
    try {
      shown = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
    } catch {
      shown = '';
    }
    if (!shown || shown.includes('\u0000')) shown = `[${type || 'binary'} file, ${Math.ceil(bytes.length / 1024)} KB; its contents are not shown, upload_file can upload it]`;
    return `<nano_file_content type="file" name="${name.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/</g, '&lt;')}">\n${shown}\n</nano_file_content>`;
  });
  return `${text}\n\n<nano_attached_files>\n${blocks.join('\n\n')}\n</nano_attached_files>`;
}
...
export class Harness {
   ...
   */
  async drive({ taskId, tabId, task, files, maxMs, maxSteps, maxInputTokens = TASK_TOKEN_CAP, allowHuman, human = [], type = 'new_task', page, during, reconnect, lateAnswer }) {
    await this.ctl.evaluate(
      ({ type, task, taskId, tabId, files }) => {
        window.__ev = [];
        window.__port.postMessage({ type, task, taskId, tabId, files });
      },
      { type, task: withAttachments(task, files), taskId, tabId, files: encodeFiles(files) },
    );
      ...
      for (const e of batch) {
        ...
        if (e.state === 'act.ask_human' && !stopReason) {
          ...
          if (script && (!script.expect || script.expect.test(question))) {
            answered++;
            await this.ctl.evaluate(
              (response, secrets, files) => window.__port.postMessage({ type: 'human_response', response, secrets, files }),
              withAttachments(script.answer, script.files),
              script.secrets ?? [],
              encodeFiles(script.files) ?? null,
            );
          } else if (!allowHuman) {
            answer = question;
...
export async function launchSession(fixtures) {
  ...
  // Downloads go to the profile's own folder without a Save As prompt, so checkers can read what was saved.
  const downloadsDir = path.join(profile, 'downloads');
  fs.mkdirSync(path.join(profile, 'Default'), { recursive: true });
  fs.writeFileSync(path.join(profile, 'Default', 'Preferences'), JSON.stringify({ download: { default_directory: downloadsDir, prompt_for_download: false } }));
  ...
  try {
    ...
    harness.downloadsDir = downloadsDir;
    return { browser, profile, harness };
  } catch (error) {
