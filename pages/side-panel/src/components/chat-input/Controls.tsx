...
export const ChatActionButtons: React.FC<ChatActionButtonsProps> = ({
        ...
        return (
            <>
                <button
                    ...
                    className={`flex items-center gap-1.5 rounded-[10px] border px-3 py-1.5 font-sans text-[11px] font-medium uppercase tracking-wider shadow-sm transition-all duration-200 active:scale-95 ${
                        isDarkMode
                            ? 'border-white/10 bg-white/[0.06] text-slate-200 hover:bg-white/10'
                            : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'
                    }`}>
                    <span>{isPaused ? t('chat_buttons_resume') : t('chat_buttons_pause')}</span>
    ...
    return (
        <button
            ...
            <span className="relative z-10">{t('chat_buttons_runTask')}</span>
